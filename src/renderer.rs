...
impl CustomProfileRenderer {
    ...
    pub fn swap_masterdata(&self, new_provider: Arc<dyn MasterDataProvider>) {
        ...
        *self
            .md_source
            .write()
            .unwrap_or_else(|e| e.into_inner()) = new_provider;
    }
...
fn render_card_encoded_with_background(
    ...
    profile: Option<&crate::profile::ProfileData>,
    clear_color: skia_safe::Color,
    personal_theme: Option<crate::personal_profile::PersonalProfileTheme>,
    format: skia_safe::EncodedImageFormat,
    quality: u32,
) -> Result<Vec<u8>, String> {
...
fn render_card_encoded_with_background_sized(
    ...
    if let Some(theme) = personal_theme {
        draw_personal_profile_general_background(
            canvas,
            theme,
            canvas_width as f32,
...
pub fn render_element_layer_cropped(
    ...
    let mut surface =
        surfaces::raster_n32_premul((w, h)).ok_or("创建 Skia Surface 失败")?;
    ...
    tracing::debug!(total = elements.len(), visible = visible_count, "图层元素扁平化完成");
    // 循环外一次性构造共享上下文，避免每个元素重建（#29）。
    let fallback_assets = crate::assets::AssetStore::new(1);
    let theme = crate::widgets::theme::Theme::default();
    for elem in &elements {
    ...
    eprintln!("[CROP-DIAG] read_pixels ok bx={bx} by={by} bw={bw} bh={bh}");
    ...
    let mut crop_surface = surfaces::raster_n32_premul((bw as i32, bh as i32))
    ...
    let encoded = crop_img
        ...
        .ok_or_else(|| {
            ...
            eprintln!("[CROP-DIAG] WebP encode FAILED crop_img size={cw}x{ch} src_rect={bx},{by},{bw},{bh}");
...
#[cfg(feature = "skia")]
fn find_opaque_bounds(pixels: &[u8], width: u32, height: u32, row_bytes: usize) -> (u32, u32, u32, u32) {
    let mut min_x = width;
...
mod tests {
    ...
    use crate::types::{
        BondsHonorEntry, BondsHonorWordEntry, CardEntry, CustomProfileCard, HonorEntry,
        ObjectData, Quaternion, StampElement, TextElement, Vec3,
    };
    ...
    impl MasterDataProvider for NullProvider {
        ...
        fn get_card(&self, _card_id: i32) -> Option<CardEntry> { None }
        fn resolve_color(&self, _color_id: i32) -> Option<ResolvedColor> { None }
        fn resolve_font(&self, _font_id: i32) -> Option<String> { None }
        fn resolve_stamp(&self, _stamp_id: i32) -> Option<String> { None }
        fn resolve_resource(&self, _res_type: &str, _id: i32) -> Option<ResourceInfo> { None }
        fn resolve_honor(&self, _honor_id: i32, _honor_level: i32) -> Option<ResolvedHonor> {
            None
        }
        fn get_bonds_honor(&self, _id: i32) -> Option<BondsHonorEntry> { None }
        fn get_bonds_honor_word(&self, _word_id: i64) -> Option<BondsHonorWordEntry> { None }
        fn get_honor(&self, _honor_id: i32) -> Option<HonorEntry> { None }
        fn resolve_unit_vs_sd(&self, _self_id: i32, _partner_id: i32) -> i32 { 0 }
        fn font_count(&self) -> usize { 0 }
        fn color_count(&self) -> usize { 0 }
    }
    ...
    fn issue5_simple_card() -> CustomProfileCard {
        CustomProfileCard {
            ...
            stamps: vec![
                StampElement { object_data: default_object_data(4, true), id: 609 },
                StampElement { object_data: default_object_data(5, true), id: 179 },
                StampElement { object_data: default_object_data(6, true), id: 631 },
                StampElement { object_data: default_object_data(7, true), id: 514 },
            ],
            texts: vec![
                TextElement {
                    object_data: default_object_data(8, false),
                    color_id: 18, font_id: 2, line_spacing: 0.0,
                    outline_color_id: 18, outline_size: 0.0, size: 24.0,
                    text: "5.9-5.12".to_string(), text_type: 513,
                },
                TextElement {
                    object_data: default_object_data(9, false),
                    color_id: 15, font_id: 2, line_spacing: 0.0,
                    outline_color_id: 15, outline_size: 0.0, size: 24.0,
                    text: "5.12-5.15".to_string(), text_type: 513,
                },
                TextElement {
                    object_data: default_object_data(10, false),
                    color_id: 17, font_id: 2, line_spacing: 0.0,
                    outline_color_id: 17, outline_size: 0.0, size: 24.0,
                    text: "5.15-5.18".to_string(), text_type: 513,
                },
                TextElement {
                    object_data: default_object_data(11, false),
                    color_id: 16, font_id: 2, line_spacing: 0.0,
                    outline_color_id: 16, outline_size: 0.0, size: 24.0,
                    text: "5.18-5.21".to_string(), text_type: 513,
                },
            ],
            card_members: vec![],
            others: vec![],
    ...
    fn stamp_layer_render_no_panic_without_assets() {
        ...
        for (z, _stamp) in card.stamps.iter().enumerate() {
            ...
            // 设置所有元素为不可见
            for s in &mut layer_card.shapes { s.object_data.visible = false; }
            for s in &mut layer_card.stamps { s.object_data.visible = false; }
            for t in &mut layer_card.texts { t.object_data.visible = false; }
            // 只设置当前 stamp 可见
    ...
    fn read_pixels_unpremul_succeeds_on_raster_surface() {
        use skia_safe::{surfaces, ColorType, AlphaType, ImageInfo};

        let w = 64i32;
        let h = 64i32;
        let mut surface = surfaces::raster_n32_premul((w, h))
            .expect("创建 raster surface 失败");
        ...
        canvas.draw_rect(
            skia_safe::Rect::from_xywh(10.0, 10.0, 40.0, 40.0),
            &paint,
        );
        ...
        let (bx, by, bw, bh) = find_opaque_bounds(&pixel_buf, w as u32, h as u32, row_bytes);
        assert!(bw > 0 && bh > 0, "find_opaque_bounds 应找到非零包围盒，实际 ({bx},{by},{bw},{bh})");
    }
