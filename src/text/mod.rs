    ...
    for (line_idx, line_str) in line_texts.iter().enumerate() {
        ...
        for (si, seg) in segments.iter().enumerate() {
            ...
            for ch in &part_chars {
                ...
                let glyph_hadv_tmp_layout =
                    tmp_measure_advance(&display, &seg_font, measure_size) * char_scale * TEXT_SCALE;
    ...
    for (i, line_str) in line_texts.iter().enumerate() {
        ...
        for (si, seg) in segments.iter().enumerate() {
            ...
            for ch in &seg_chars {
                ...
                let ft_advance_x = sdf_glyph
                    .as_ref()
                    .map(|g| g.plane_advance_x() * ft_scale);
                let ft_pivot_x = sdf_glyph.as_ref().map(|g| {
                    (g.plane_bearing_x() + g.plane_width() / 2.0) * ft_scale
                });
                ...
                let ft_pivot_y = sdf_glyph.as_ref().map(|g| {
                    -(g.plane_bearing_y() - g.plane_height() / 2.0) * ft_scale
                });
