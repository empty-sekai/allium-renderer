...
fn render_live_board(input: &RankingLiveBoardInput) -> Result<RenderOutput, RenderError> {
    ...
    let height =
        (header_h + 34.0 + row_h * player_rows as f32 + bottom_h + 32.0 + FOOTER_HEIGHT as f32)
            .ceil()
            .max(360.0) as u32;
...
fn render_border_summary(input: &RankingBorderSummaryInput) -> Result<RenderOutput, RenderError> {
    ...
    let header_h = 146.0;
    let height =
        (header_h + 34.0 + row_h * input.borders.len() as f32 + 32.0 + FOOTER_HEIGHT as f32)
            .ceil()
            .max(330.0) as u32;
...
fn render_top100_compact_skia(
    ...
    canvas.glass(
        ...
        panel_y,
        panel_w,
        height as f32 - panel_y - 32.0 - FOOTER_HEIGHT as f32,
    );
...
fn render_border_summary_skia(
    ...
    draw_border_table_columns(&mut canvas, table_x + 24.0, header_h - 10.0, table_w - 48.0);
...
fn render_speed_borders(input: &RankingSpeedBordersInput) -> Result<RenderOutput, RenderError> {
    ...
    let header_h = 146.0;
    let height =
        (header_h + 34.0 + row_h * input.borders.len() as f32 + FOOTER_HEIGHT as f32 + 32.0)
            .ceil()
            .max(330.0) as u32;
...
fn render_speed_detail_skia(
    ...
    let chips = [
        (
            "最近一轮",
            &format_w(p.projected_20m_x3 / 3.0),
            rate_color(p.projected_20m_x3 / 3.0, p.tracking_window_secs),
        ),
        (
            "1h名次",
            &display_rank_delta_1h(
                p.rank_delta_1h,
                p.rank_delta_1h_estimated,
                p.tracking_window_secs,
            ),
            rank_delta_color(p.rank_delta_1h),
        ),
        (
            "时速",
            &display_hourly_rate(p.hourly_rate, p.tracking_window_secs),
            rate_color(p.hourly_rate, p.tracking_window_secs),
        ),
    ];
    ...
    let mini_chips: [(&str, &str, Color); 5] = [
        ...
        (
            "20m×3",
            &display_hourly_rate(p.projected_20m_x3, p.tracking_window_secs),
            rate_color(p.projected_20m_x3, p.tracking_window_secs),
        ),
...
fn render_speed_borders_skia(
    ...
    draw_speed_border_table_columns(&mut canvas, table_x + 24.0, header_h - 10.0, table_w - 48.0);
...
fn render_daily_detail_skia(
    ...
    let chips = [
        (
            "日速",
            &display_daily_rate(p.daily_rate, p.daily_tracking_window_secs, p.is_fallback),
            rate_color(p.daily_hourly_rate, p.daily_tracking_window_secs),
        ),
        (
            "日均时速",
            &display_hourly_rate(p.daily_hourly_rate, p.daily_tracking_window_secs),
            rate_color(p.daily_hourly_rate, p.daily_tracking_window_secs),
        ),
        (
            "24h名次",
            &display_rank_delta_1h(
                p.rank_delta_24h,
                p.rank_delta_24h_estimated,
                p.daily_tracking_window_secs,
            ),
            rank_delta_color(p.rank_delta_24h),
        ),
        (
            "24h增量",
            &format_signed(p.daily_score_delta),
            delta_color(p.daily_score_delta),
        ),
    ];
    for (i, (label, value, color)) in chips.iter().enumerate() {
        draw_stat_chip(
    ...
    let mini_chips: [(&str, &str, Color); 2] = [
        (
            "内存时速",
            &display_hourly_rate(p.hourly_rate, 3600),
            rate_color(p.hourly_rate, 3600),
        ),
        (
            "窗口",
            &format_window(p.daily_tracking_window_secs),
            Color::MUTED,
        ),
    ];
    for (i, (label, value, color)) in mini_chips.iter().enumerate() {
...
fn render_daily_borders_skia(
    ...
    draw_daily_border_table_columns(&mut canvas, table_x + 24.0, header_h - 10.0, table_w - 48.0);
...
fn render_daily_compare_skia(
    ...
    for (i, subject) in input.subjects.iter().enumerate() {
        ...
        let rate_text = display_daily_rate(
            subject.daily_rate,
            subject.daily_tracking_window_secs,
            subject.is_fallback,
        );
        canvas.text(
            &rate_text,
