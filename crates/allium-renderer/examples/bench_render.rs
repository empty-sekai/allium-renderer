...
impl MasterDataProvider for CachedProvider {
    fn resolve_story_banner(&self, story_type: &str, story_id: i32) -> Option<String> {
        ...
        match story_type {
            ...
            "unit_story" => {
                let abn =
                    self.table("unitStoryEpisodeGroups")?.by_id(id)?["assetbundleName"].as_str()?;
    ...
    fn resolve_honor(&self, honor_id: i32, honor_level: i32) -> Option<ResolvedHonor> {
        ...
        let (abn, rarity) = if is_live_master {
            ...
            (
                lvl.and_then(|l| l.assetbundle_name.as_deref())
                    .unwrap_or("")
                    .to_string(),
                lvl.and_then(|l| l.honor_rarity.as_deref())
                    .unwrap_or("low")
                    .to_string(),
            )
        } else {
        ...
        let group: Option<HonorGroupEntry> = honor
            .group_id
            .and_then(|g| self.typed("honorGroups", g as i64));
        Some(ResolvedHonor {
    ...
    fn resolve_unit_vs_sd(&self, self_id: i32, partner_id: i32) -> i32 {
        ...
        for e in units.all() {
            if e["gameCharacterId"].as_i64().unwrap_or(0) == self_cid
                && e["unit"].as_str().unwrap_or("") == target
            {
...
fn main() -> Result<(), String> {
    ...
    let input = PathBuf::from(
        args.next()
            .unwrap_or_else(|| "tmp/render_cache/raw_case1.json".to_string()),
    );
    ...
    let provider = Arc::new(CachedProvider::from_cache_dir(Path::new(
    ...
    let mut cards: Vec<UserCustomProfileCard> = serde_json::from_value(
        body.get("userCustomProfileCards")
            .cloned()
            .ok_or("missing userCustomProfileCards")?,
    )
    ...
    let card = cards
        .iter()
        .find(|c| c.seq == seq)
        .ok_or_else(|| format!("seq {seq} not found"))?;
    ...
    let elem_count = {
        ...
        c.texts.len()
            ...
            + c.generals.len()
            + c.stand_members.len()
            + c.general_backgrounds.len()
            + c.story_backgrounds.len()
    };
    ...
    println!("\n=== 阶段分解（mean，隔离测量）===");
    println!(
        "flatten_and_sort: {flat_mean:.4}ms  ({:.1}% of 总)",
        pct(flat_mean, mean)
    );
    println!(
        "draw_element 循环: {loop_mean:.4}ms  ({:.1}% of 总)",
        pct(loop_mean, mean)
    );
    println!(
        "其余(surface+snapshot+encode): {:.4}ms  ({:.1}% of 总)",
        mean - flat_mean - loop_mean,
        pct(mean - flat_mean - loop_mean, mean)
    );
